...
import { ZennRssArticleCreateInput, ZennTopic } from "../../types";

export const transformZennRssArticles = async (): Promise<ZennRssArticleCreateInput[]> => {
    ...
    try {
        ...
        const itemBlocks = xml.split(`<${blockTag}>`).slice(1);
        ...
        const items: ZennRssArticleCreateInput[] = [];

        for (const block of itemBlocks) {
            const itemXml = block.split(`</${blockTag}>`)[0];
            const rawUrl =
                itemXml.match(/<link[^>]*>(.*?)<\/link>/s)?.[1] ||
                itemXml.match(/<link[^>]+href="([^"]+)"/)?.[1] ||
                itemXml.match(/<guid[^>]*>(.*?)<\/guid>/s)?.[1] ||
                "";
        ...
        console.error("[Zenn Transform] failed", error);
...
function extractZennTags(html: string): string | null {
    const matches = [...html.matchAll(/"topics":(\[.*?\])/gs)];

    if (!matches.length) {
        console.log("[Zenn Tag Extract] not found");

        return null;
    }

    try {
        const topics: ZennTopic[] = JSON.parse(matches[0][1]);

        const tags = topics
            .map((topic) => topic.name || topic.slug)
            .filter((tag): tag is string => Boolean(tag));

        console.log("[Zenn Tag Extract] parsed", {
            tags,
        });

        return tags.join(",");
    } catch (e) {
        console.error("[Zenn Tag Extract] parse failed", e);
